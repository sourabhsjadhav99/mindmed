...
const Carousel = ({ data }) => {
  ...
  return (
    <div className="w-[95%] mx-auto pb-10">
      <h2 className="text-2xl text-white font-semibold text-center mb-5">{data.title}</h2>
      <Slider {...settings}>
        {data?.teamMembers?.map((member, index) => (
          <div key={index} className="p-4">
            <div className="bg-white rounded-lg shadow-lg p-6 flex flex-col items-center">
              ...
              />
              <h3 className="text-lg md:text-xl font-semibold  line-clamp-1">{member.name}</h3>
